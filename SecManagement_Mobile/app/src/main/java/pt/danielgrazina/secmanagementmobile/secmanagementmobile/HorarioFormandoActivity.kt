...
class HorarioFormandoActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_horario_turma) // podes reutilizar o mesmo layout

        formandoId = intent.getIntExtra("formandoId", -1)
        ...
        adapter = HorarioSemanaAdapter(emptyList()) { /* se quiseres detalhe aqui */ }
        rv.adapter = adapter
        ...
        scope.launch {
            try {
                val list = withContext(Dispatchers.IO) {
                    val token = TokenStore.get(this@HorarioFormandoActivity) ?: ""
                    val auth = "Bearer $token"
                    ApiClient.api.getHorarioFormando(auth, formandoId, startStr, endStr)
                }
